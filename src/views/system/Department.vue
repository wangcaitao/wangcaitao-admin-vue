...
import DepartmentBO from "@/views/system/entity/DepartmentBO"
import DepartmentDTO from "@/views/system/entity/DepartmentDTO"
...
export default class Department extends Vue {
    ...
    rows = 0
    // 表格数据
    tableData: DepartmentBO[] = []
    // 是否显示新增按钮
    showAddButton = AuthorizeUtils.hasAuthority("POST/department")
    // 是否显示修改按钮
    showEditButton = AuthorizeUtils.hasAuthority("PUT/department")
    // 是否显示删除按钮
    showDeleteButton = AuthorizeUtils.hasAuthority("DELETE/department/{id}")
    // 是否显示修改排序按钮
    showUpdateSortButton = AuthorizeUtils.hasAuthority("PUT/department-sort")
    // 是否显示关联员工按钮
    showRelationEmployeeButton = AuthorizeUtils.hasAuthority("GET/department-employee")
    // 操作列宽度
    ...
    search(pageNum: number) {
        ...
        AxiosUtils.get("/department", { params: this.queryModel }).then((response) => {
            const result = response as Result<DepartmentBO[]>
    ...
    handleEdit(id: number) {
        ...
        AxiosUtils.get("/department/" + id).then((response) => {
            this.dialogData = (response as Result<DepartmentDTO>).data
        })
    }
    ...
    handleDelete(id: number) {
        this.$confirm("确定删除?", "提示", {
            ...
            .then(() => {
                AxiosUtils.delete("/department/" + id).then(() => {
    ...
    handlerUpdateSort(id: number, moveTypeCode: number) {
        AxiosUtils.put("/department-sort", {
            id: id,
        ...
        ;(this.$refs["dialogForm"] as Form).validate((valid: boolean) => {
            if (valid) {
                if (this.dialogData.id == 0) {
                    ...
                    AxiosUtils.post("/department", this.dialogData).then(() => {
                        this.$message.success(process.env.VUE_APP_SUCCESS_MESSAGE)
                        this.dialogClose()
                        this.changedReload(1)
                    })
                } else {
                    AxiosUtils.put("/department", this.dialogData).then(() => {
                        this.$message.success(process.env.VUE_APP_SUCCESS_MESSAGE)
                        this.dialogClose()
                        this.changedReload(this.queryModel.pageNum)
                    })
