<template>
    <div>
        ...
        <el-table :data="tableData" stripe border max-height="520px" @selection-change="handleSelectionChange">
            ...
            <el-table-column header-align="center" align="center" label="职位" prop="positionName" min-width="100px" v-if="relationStatusCode == '1'" />
...
import PositionEmployeeBO from "@/views/system/entity/PositionEmployeeBO"

@Component
export default class PositionEmployee extends Vue {
    // 加载中
    ...
    // 是否显示批量删除按钮
    showBatchDeleteButton = AuthorizeUtils.hasAuthority("DELETE/position-employee")
    // 是否显示批量关联按钮
    showBatchRelationButton = AuthorizeUtils.hasAuthority("POST/position-employee")

    /**
     * created
     */
    created() {
        this.positionId = Number(this.$route.query.positionId)
        if (isNaN(this.positionId)) {
    ...
    search(pageNum: number) {
        ...
        this.queryModel.positionId = this.positionId
        this.queryModel.relationStatusCode = this.relationStatusCode
        AxiosUtils.get("/position-employee", { params: this.queryModel }).then((response) => {
            const result = response as Result<PositionEmployeeBO[]>
     ...
     * 表格多选
     */
    handleSelectionChange(selectedRows: PositionEmployeeBO[]) {
        this.selectedIds = []
    ...
    handleBatchRelation() {
        this.$confirm("确定关联?", "提示", {
            ...
            .then(() => {
                AxiosUtils.post("/position-employee", {
    ...
    handleBatchDelete() {
        this.$confirm("确定删除?", "提示", {
            ...
            .then(() => {
                AxiosUtils.delete("/position-employee", {
